...
const centerBlockEl = document.querySelector('.center-block');
...
// formatTime() lives in clock.js so it can be tested independently.
...
async function initSettings() {
  const [showClock, showGreeting, showLocation, clockFormat] = await Promise.all([
  ...
  const clockFmtToggle = document.getElementById('toggle-clock-format');
  const clockFmtRow = clockFmtToggle.closest('.settings-row');
  ...
  clockFmtToggle.addEventListener('click', async () => {
    ...
    await setSetting('clockFormat', _clockFormat);
  });
  ...
  document.getElementById('toggle-clock').addEventListener('click', () => {
    const clockVisible = document.getElementById('toggle-clock').getAttribute('aria-checked') === 'true';
