...
import type { Photo, TimePeriod, PhotoMeta } from './types.js';
...
let _photoCache: PhotoMeta | null = null;

async function getPhotoMeta(): Promise<PhotoMeta> {
  ...
  _photoCache = await res.json() as PhotoMeta;
  return _photoCache;
}
...
async function getNextPhoto(): Promise<Photo> {
...
export async function getNextImage(): Promise<Photo> {
  ...
  if (preloaded) {
    ...
    void preloadNextImage(); // fire-and-forget: runs in background, doesn't block
    return preloaded;
  }
